...
function DynamicImageGrid({ embeds, indices, embedMetadata, onImageClick }: DynamicImageGridProps) {
  ...
  const extractImageUrl = (embed: any): { imageUrl: string | null; linkUrl: string } => {
    ...
    // Filter out favicon images - hide instead of showing placeholder
    if (imageUrl && (imageUrl.includes('/favicon.') || imageUrl.endsWith('/favicon.png') || imageUrl.endsWith('/favicon.ico'))) {
      imageUrl = null;
    }
...
export function CastCard({ cast, showThread = false, showTopReplies = true, onUpdate, feedType, curatorInfo, sortBy, isReply = false, disableClick = false, rootCastHash, compressedView = false, displayMode }: CastCardProps) {
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      console.log("[Like Frontend] Starting like toggle:", {
        castHash: cast.hash,
        wasLiked,
      ...
      console.log("[Like Frontend] Sending request:", {
        url: "/api/reaction",
        method: wasLiked ? "DELETE" : "POST",
        body: requestBody,
      });
      ...
      console.log("[Like Frontend] Received response:", {
      ...
      if (!response.ok) {
        ...
        console.error("[Like Frontend] Request failed:", {
          status: response.status,
          error: errorData,
          castHash: cast.hash,
        });
      ...
      console.log("[Like Frontend] Request successful:", {
        castHash: cast.hash,
        wasLiked,
      ...
      console.error("[Like Frontend] Error in handleLike:", {
        castHash: cast.hash,
        error: error.message,
        stack: error.stack,
      });
  ...
  // Helper function to get quality color based on score (light tan for low, dark tan for high)
  const getQualityColor = (score: number): string => {
    // Clamp score between 0 and 100
    const clampedScore = Math.max(0, Math.min(100, score));
    
    // Map score to tan color gradient:
    // 0-20: Very light tan
    // 20-40: Light tan
    // 40-60: Medium tan
    // 60-80: Medium-dark tan
    // 80-100: Dark tan
    // Use darker text colors for better contrast
    
    if (clampedScore < 20) {
      return "text-gray-900 dark:text-gray-100 bg-accent/15 dark:bg-accent/10";
    } else if (clampedScore < 40) {
      return "text-gray-900 dark:text-gray-100 bg-accent/25 dark:bg-accent/15";
    } else if (clampedScore < 60) {
      return "text-gray-900 dark:text-gray-100 bg-accent/40 dark:bg-accent/25";
    } else if (clampedScore < 80) {
      return "text-gray-900 dark:text-gray-100 bg-accent/60 dark:bg-accent-dark/40";
    } else {
      return "text-gray-900 dark:text-gray-100 bg-accent-dark/80 dark:bg-accent-dark/60";
    }
  };
  ...
  const getQualityTextColor = (score: number): string => {
    ...
    // Use darker tan text for higher scores
    if (clampedScore < 60) {
      return "text-accent-dark dark:text-accent";
    } else {
      return "text-accent-dark dark:text-accent-dark";
    }
  };
            ...
            <div className="flex items-center gap-3 sm:gap-6 mt-3 sm:mt-4 flex-wrap" onClick={(e) => e.stopPropagation()}>
              ...
              {isCurated && (
                <button
                  ...
                  className={`flex items-center gap-1 sm:gap-2 text-xs sm:text-sm transition-all py-1 px-1 sm:px-0 disabled:opacity-50 disabled:cursor-not-allowed ${showThankAnimation ? "thank-animate" : ""}`}
                >
                  <svg 
