...
export async function GET(request: NextRequest) {
  try {
    ...
    if (enrichedContributors.length <= 10) {
      ...
      body = `Weekly Contributors: ${curatorNames.join(", ")}, and ${remaining} more curator${remaining === 1 ? '' : 's'}`;
    }
