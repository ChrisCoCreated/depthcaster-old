...
export async function GET(request: NextRequest) {
  try {
    ...
    const allCollections = await db.select().from(collections).orderBy(collections.createdAt);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { adminFid, name, displayName, description, accessType, gatedUserId, gatingRule, displayType, autoCurationEnabled, autoCurationRules, displayMode, headerConfig } = body;
