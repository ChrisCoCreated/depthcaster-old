...
export async function POST(
  ...
  try {
    ...
    // Check permissions - allow if user is creator OR has collector/admin role
    const collection = await db.select().from(collections).where(eq(collections.name, name)).limit(1);
    if (collection.length === 0) {
      return NextResponse.json({ error: "Collection not found" }, { status: 404 });
    }

    const isCreator = collection[0].creatorFid === adminFidNum;
    const roles = await getUserRoles(adminFidNum);
    const hasRole = hasCollectionsOrAdminRole(roles);
    
    if (!isCreator && !hasRole) {
      return NextResponse.json({ error: "User does not have permission to auto-curate this collection" }, { status: 403 });
    }

    // Check if auto-curation is enabled
    if (!collection[0].autoCurationEnabled) {
      return NextResponse.json({ error: "Auto-curation is not enabled for this collection" }, { status: 400 });
    }

    // Process the collection
    const result = await processCollection(collection[0]);
    ...
    return NextResponse.json({
      ...
      message: `Auto-curation completed: ${result.added} cast(s) added, ${result.errors} error(s)`,
    });
  } catch (error: unknown) {
    const err = error as { message?: string };
    console.error("Auto-curate API error:", err.message || error);
    return NextResponse.json(
      { error: err.message || "Failed to run auto-curation" },
      { status: 500 }
    );
  }
}
